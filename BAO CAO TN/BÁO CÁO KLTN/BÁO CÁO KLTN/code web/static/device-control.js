...
function initializeTimerInputs(device) {
    ...
    const handleInputFocus = () => {
        if (inputStates[device].timeoutId) clearTimeout(inputStates[device].timeoutId);
        inputStates[device].isEditing = true;
        console.log(`Started editing timer for ${device}`);
    };

    const handleInputBlur = () => {
        inputStates[device].timeoutId = setTimeout(() => {
            inputStates[device].isEditing = false;
            console.log(`Stopped editing timer for ${device}`);
            updateDeviceUI(device);
        }, 10000);
    };
    ...
    const handleInputChange = () => {
        ...
        inputStates[device].timeoutId = setTimeout(() => {
            inputStates[device].isEditing = false;
            console.log(`Stopped editing timer for ${device} after change`);
            updateDeviceUI(device);
        }, 10000);
    };

    if (onTimeInput) {
        onTimeInput.addEventListener('focus', handleInputFocus);
        onTimeInput.addEventListener('blur', handleInputBlur);
        onTimeInput.addEventListener('change', handleInputChange);
    }
    if (offTimeInput) {
        offTimeInput.addEventListener('focus', handleInputFocus);
        offTimeInput.addEventListener('blur', handleInputBlur);
        offTimeInput.addEventListener('change', handleInputChange);
    }
    if (onDateInput) {
        onDateInput.addEventListener('focus', handleInputFocus);
        onDateInput.addEventListener('blur', handleInputBlur);
        onDateInput.addEventListener('change', handleInputChange);
    }
    if (offDateInput) {
        offDateInput.addEventListener('focus', handleInputFocus);
        offDateInput.addEventListener('blur', handleInputBlur);
        offDateInput.addEventListener('change', handleInputChange);
    }
}
    ...
    setInterval(async () => {
        ...
        for (const device of devices) {
            try {
                ...
                console.log(`Periodic timer check for ${device}:`, data);
